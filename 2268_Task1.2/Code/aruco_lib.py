...
def detect_Aruco(img,g_size,com):  #returns the detected aruco list dictionary with id: corners
    aruco_list = {}
    gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
    if(g_size == 4 and com == 100): #To automatically select the dictionary according to the image
        aruco_dict = aruco.Dictionary_get(aruco.DICT_4X4_100)  #For 4X4 grid and 100 combinations
    if(g_size == 5 and com == 1000):
        aruco_dict = aruco.Dictionary_get(aruco.DICT_5X5_1000) #For 5X5 grid and 1000 combinations
    if(g_size == 7 and com == 250):
        aruco_dict = aruco.Dictionary_get(aruco.DICT_7X7_250)  #For 7X7 grid and 100 combinations
    print(aruco_dict)
